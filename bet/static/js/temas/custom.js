...
(function () {
  ...
  window.addEventListener("load", function (event) {
   // trocar o tema por seleçao
    const themeSelect = document.getElementById("theme");
    try {
      themeSelect.addEventListener("change", function () {
        const selectedTheme = getSelectedTheme();
        document.querySelector("head > link:nth-child(10)").href = `static/css/${selectedTheme}.css`;
        console.log(selectedTheme);
      });
    }
    catch {  }
    function getSelectedTheme() {
      const selectedIndex = themeSelect.selectedIndex;
      const selectedOption = themeSelect.options[selectedIndex];
      return selectedOption.value;
    }

    //esconder menu 
    const toggleMenu = document.getElementsByClassName("navbar-toggler");
    const btCanvas = document.getElementById("offcanvas-start");
    toggleMenu[0].addEventListener("click", function () {
      let estado = toggleMenu[0].classList.contains("collapsed")
      if (!estado) {
        btCanvas.style.display = "none";
      }
      else { btCanvas.style.display = "block"; }
    });
  });
